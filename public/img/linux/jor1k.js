...
Filesystem.prototype.TAR = function(path) {
    ...
    message.Register("tar", function(d){
      download(d, arrayPath[arrayPath.length-1]+".tar", "application/x-tar");
...
Filesystem.prototype.OnSync = function(d) {
    utils.UploadBinaryResource(this.syncURL, this.userid + ".tar", d, function(response) {
            alert(
...
Framebuffer.prototype.SetFPS = function(fps) {
    ...
    if (fps != 0) 
    {
        this.fbinterval = window.setInterval(function(){
          message.Send("GetFB", 0)
...
Framebuffer.prototype.Update = function(buffer) {
    ...
    //data.set(buffer);
    this.fbctx.putImageData(this.fbimageData, 0, 0); 
    // at coords 0,0
}
...
function SetWorker(_worker) {
    ...
    Register("Abort", function() {
      Debug("Master: Received abort signal from worker"); 
      run=false;
    });
    Register("Debug", function(d) {
      Debug(d);
    });
}
...
function jor1kGUI(parameters) {
    ...
    // ----------------------
    this.worker = (this.params.worker instanceof Worker) ? this.params.worker : new Worker("worker.js");
    ...
    {
        this.terms = this.params.terms.slice(0, 2); 
        // support up to 2 terminals
    }
   ...
   this.IgnoreKeys = function() {
    ...
    if(document.addEventListener)
      ...
      Window.onmousedown = recordTarget;
      // IE 10 support (untested)
...
jor1kGUI.prototype.ShowIPS = function(ips) {
    ...
    {
        this.stats.innerHTML = ips < 1000000 ? Math.floor(ips/1000) + " KIPS" : (Math.floor(ips/100000)/10.) + " MIPS";
   }
};
...
}],"LinuxTerm":[function(require,module,exports){
